...
const tyrannosaurus = {
  ...
  roar() {
    return "RAWERSRARARWERSARARARRRR!"; // Causes the tyrannosaurus to roar. used console.log on line 55.
  }
};
...
console.log(tyrannosaurus.roar()); //used roar method from line 17.
...
graduates.forEach(element => {
  // element is for each universities.
  universities.push(element.university);
  universities.sort();
});
console.log(universities.join()); // Used join to list them next to eachother.
...
zooAnimals.forEach(element => {
  displayNames.push(
    `Name: ${element.animal_name}, ${element.scientific_name}.`
  );
});
...
populationTotal = zooAnimals.reduce(
  (addition, animals) => addition + animals.population,
  0
);
